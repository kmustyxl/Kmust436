...
def Bayes_update(Mat_pop, factory_job_set, num_factory):
    prob_mat_first = [[0.0001 for i in range(len(factory_job_set[k]))] for k in range(num_factory)]
    #返回每个工厂相邻两个工件的所有情况的概率分布
    for i in range(num_factory):
        job_len = len(factory_job_set[i])
        #确定数据中第一个工件的出现概率
        index = 0
        demo = [ii[0] for ii in Mat_pop[i][0:200]]
        for job in factory_job_set[i]:
            prob_mat_first[i][index] = demo.count(job) / 200
            index += 1
...
def New_pop(prob_mat_first, num_factory, factory_job_set, Mat_pop):
    #每个工厂每次更新20个个体
    newpop = [[0 for i in range(len(factory_job_set[k]))] for k in range(num_factory)]
    #三维数组--第一维：工厂数；第二维：每个工厂的所有相邻关系数组；第三维：与上一个工件的关系
    prob_mat = [[[0.0001 for i in range(len(factory_job_set[k]))]
    ...
    for i in range(num_factory):
        ...
        if r > prob_mat_first[i][0]:
            newpop[i][0] = factory_job_set[i][0]
        else:
            j = 0
            ...
            while temp < r:
                j += 1
                temp += prob_mat_first[i][j]
            newpop[i][0] = factory_job_set[i][j - 1]
        for k in range(1, job_len):
            temp_job = []
            for ii in Mat_pop[i][0:200]:
                if ii[k] in newpop[i]:
                    continue
                elif ii[k - 1] == newpop[i][k - 1]:
            ...
            while temp < r:
                ...
                if j == len(factory_job_set[i]):
                        break
            newpop[i][k] = factory_job_set[i][j - 1]
    return newpop

def local_search(newpop, ls_frequency, factory_job_set, num_factory):
    ls_pop = [[[0 for i in range(len(factory_job_set[k]))] for j in range(ls_frequency)] for k in range(num_factory)]
    for i in range(num_factory):
        ls_pop[i][0][:] = newpop[i][:]
